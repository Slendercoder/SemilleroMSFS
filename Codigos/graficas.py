...
# data['AvScorPolRond'] = data.groupby(['Ronda', 'Politica'])['Puntaje'].transform('mean')
# print(data[['Politica', 'Ronda', 'AvScorPolRond']][:10])
#
# # ax = data.groupby('Politica').plot(x='Ronda', y='AcScorPolRond', marker='o')
#
# for key, grp in data.groupby('Politica'):
#     print('Politica', key)
#     print(grp['AvScorPolRond'][:10])
#     ax = grp['AvScorPolRond'].plot(label=key)
#
# plt.show()
...
# data.hist('Politica', align='mid', bins=bins, grid=False, ax=ax)
...
# USO-Politica = Numero de agentes que uso la estrategia
# GRAFICA_USO-Politica_VS_RONDA

# GRAFICA_PUNTAJE-Promedio-Politica_VS_RONDA

# GRAFICA_ASISTENTES_BAR-Promedio-por-Simulacion_VS_RONDA

# HISTOGRAMA_USO iteracion i por todas las simulaciones
